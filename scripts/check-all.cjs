...
function generateSummary(results) {
  ...
  const projectsWithUpdates = results.filter(r =>
    r.statusUpdates && Object.keys(r.statusUpdates).length > 0
  ).length;
...
common.processProjects(
  ...
  ['Project Name', 'Package Version', 'Project Type', 'Project Path', 'find-projects', 'check-gitignore', 'check-memory-bank', 'check-scripts'],
  generateRow,
  generateSummary,
