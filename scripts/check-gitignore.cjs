...
function checkGitignore(projectPath) {
    ...
    try {
        ...
        const hasAllLines = referenceGitignoreLines.every((line) => gitignoreLines.includes(line));
...
function updateGitignore(projectPath) {
    ...
    try {
        ...
        if (missingLines.length > 0) {
            ...
            console.log(
                `Updated .gitignore for ${path.basename(projectPath)} with ${missingLines.length} missing lines`
            );
        } else {
...
function generateSummary(results) {
    return {
        ...
        'Projects with non-matching .gitignore': results.filter(
            (r) => !r.gitignoreStatus && !r.isExcluded
        ).length,
