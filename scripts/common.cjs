...
function updateBpStatus(bpStatusPath, statusField) {
    try {
        ...
        console.log(
            `Updated bpstatus.json at ${bpStatusPath} with version ${currentVersion} and ${statusField} date ${formattedDate}`
        );
...
function copyFile(sourcePath, destDir, destFileName) {
    try {
        ...
        console.error(
            `Error copying file ${sourcePath} to ${destDir}/${destFileName}: ${error.message}`
        );
        return false;
    }
}
...
function loadEnvironmentVariables() {
    ...
    if (!projectsFolders) {
        ...
        console.log(
            'Please add PROJECTS_FOLDERS to your .env file with comma-separated paths to search'
        );
    ...
    try {
        if (fs.existsSync(bpStatusPath)) {
            ...
            try {
                ...
                console.error(
                    `Error parsing bpstatus.json at ${bpStatusPath}: ${parseError.message}`
                );
                return {};
            }
        }
...
function processProjects(
    ...
    for (const project of allProjects) {
        ...
        if (isExcluded) {
            console.log(
                `${project.projectName}: Script ${scriptName} is excluded by bpstatus.json exclude rules, skipping...`
            );
        } else {
            // Process the project
            ...
            const changesWereMade = Object.keys(processorResult).some(
                (key) => key.endsWith('Updated') && processorResult[key] === true
            );
            ...
            if (changesWereMade) {
                ...
                console.log(
                    `${project.projectName}: No changes made, skipping bpstatus.json update`
                );
