...
function updatePackageJsonScripts(projectPath) {
    ...
    try {
        ...
        Object.keys(packageJson.scripts).forEach((key) => {
            if (key.startsWith('bp:')) {
                if (bpScripts[key] !== packageJson.scripts[key]) {
                    ...
                    console.log(
                        `Removed old ${key} script from ${path.basename(projectPath)}/package.json`
                    );
                }
            }
        });
        ...
        if (!scriptsChanged) {
            console.log(
                `No changes needed for bp:* scripts in ${path.basename(projectPath)}/package.json`
            );
        }
        ...
        if (packageJsonUpdated) {
            ...
            console.log(
                `Updated package.json for ${path.basename(projectPath)} with ${scriptsAdded} bp:** scripts`
            );
        }
...
function cleanReleaseScripts(projectPath, projectType) {
    ...
    if (packageJsonUpdated) {
        ...
        console.log(
            `Updated package.json for ${path.basename(projectPath)}, removed scripts: ${scriptsRemoved.join(', ')}`
        );
    } else {
        console.log(
            `No scripts needed to be removed from ${path.basename(projectPath)}/package.json`
        );
    }
...
function generateSummary(results) {
    return {
        ...
        'Total BP scripts added to package.json': results.reduce(
            (sum, r) => sum + r.scriptsAdded,
            0
        ),
        'Total BP scripts removed from package.json': results.reduce(
            (sum, r) => sum + (r.scriptsRemoved ? r.scriptsRemoved.length : 0),
            0
        ),
